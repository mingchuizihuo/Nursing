 ...
 */
var url = "/serve/fixed_assets_type"
/*财产类别*/
//添加
$(function () {
    findAll(currentPage);
...
function add() {
    ...
    var postData = {
        fixedAssetsTypeName:$("#fixedTypeName").val(),
        tid:$("#classTid").val(),
    };
    ...
    getAjax(urlFindAll,false,getData,function (data) {
        ...
        for (var i = 0; i < num1; i++) {
            html += '<li style="border-top:1px solid #dddddd"><a href="#" class="one"><i class="glyphicon glyphicon-plus"></i>' +
                '' + data.aaData[0][i].fixedAssetsTypeName + '</a><cc><a' + ' onclick="openModal(' + data.aaData[0][i].id + ')">添加子类</a>' +
            ...
            if (num2 >= 1) {
                ...
                for (var j = 0; j < num2; j++) {
                    if (data.aaData[0][i].id == data.aaData[1][j].tid) {
                        html += '<li><a href="#" class="one">' + data.aaData[1][j].fixedAssetsTypeName + '</a><cc>' +
